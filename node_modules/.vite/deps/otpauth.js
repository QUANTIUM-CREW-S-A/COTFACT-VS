...
function a(t2, n2) {
  ...
  if (i2 % 4 != 0) {
    ...
    return (t2.value.length << 2) - 4 >= s2 + i2 && t2.value.pop(), {
      value: t2.value,
      binLen: t2.binLen + n2.binLen
    };
  }
  return {
    value: t2.value.concat(n2.value),
    binLen: t2.binLen + n2.binLen
  };
}
function l(t2) {
  const n2 = {
    ...
    b64Pad: "=",
    outputLen: -1
  }, e2 = t2 || {}, r2 = "Output length must be a multiple of 8";
...
function rt(t2) {
  ...
  return r2 = 0 !== r2 ? r2 : 1, i2[0] |= r2, {
    value: r2 + 1 > 4 ? i2 : [i2[0]],
    binLen: 8 + 8 * r2
  };
}
function it(t2) {
...
var ot = class extends E {
  ...
  O(t2, n2) {
    const e2 = function(t3) {
      const n3 = t3 || {};
      return {
        funcName: A("funcName", n3.funcName, 1, {
          value: [],
          binLen: 0
        }),
        customization: A("Customization", n3.customization, 1, {
          value: [],
          binLen: 0
        })
      };
  ...
  X(t2) {
    const n2 = function(t3) {
      const n3 = t3 || {};
      return {
        kmacKey: A("kmacKey", n3.kmacKey, 1),
        funcName: {
          value: [1128353099],
          binLen: 32
        },
  ...
  _(t2) {
    const n2 = a({
      ...
      return r2 = 0 !== r2 ? r2 : 1, i2[r2 >> 2] |= r2 << 8 * r2, {
        value: r2 + 1 > 4 ? i2 : [i2[0]],
        binLen: 8 + 8 * r2
      };
    }(t2.outputLen));
...
var base32ToBuf = (str) => {
  ...
  let value = 0;
  let index = 0;
  for (let i2 = 0; i2 < cstr.length; i2++) {
    const idx = ALPHABET.indexOf(cstr[i2]);
    if (idx === -1) throw new TypeError(`Invalid character found: ${cstr[i2]}`);
    value = value << 5 | idx;
    bits += 5;
    if (bits >= 8) {
      bits -= 8;
...
var timingSafeEqual = (a2, b2) => {
  {
    ...
    let out = 0;
    while (++i2 < a2.length) {
      out |= a2.charCodeAt(i2) ^ b2.charCodeAt(i2);
    }
    return out === 0;
  }
};
var HOTP = class _HOTP {
  ...
  static validate({
    ...
    let delta = null;
    for (let i2 = counter - window2; i2 <= counter + window2; ++i2) {
  ...
  toString() {
    ...
    return `otpauth://hotp/${this.issuer.length > 0 ? `${e2(this.issuer)}:${e2(this.label)}?issuer=${e2(this.issuer)}&` : `${e2(this.label)}?`}secret=${e2(this.secret.base32)}&algorithm=${e2(this.algorithm)}&digits=${e2(this.digits)}&counter=${e2(this.counter)}`;
  }
};
var TOTP = class _TOTP {
  ...
  toString() {
    const e2 = encodeURIComponent;
    return `otpauth://totp/${this.issuer.length > 0 ? `${e2(this.issuer)}:${e2(this.label)}?issuer=${e2(this.issuer)}&` : `${e2(this.label)}?`}secret=${e2(this.secret.base32)}&algorithm=${e2(this.algorithm)}&digits=${e2(this.digits)}&period=${e2(this.period)}`;
  }
};
